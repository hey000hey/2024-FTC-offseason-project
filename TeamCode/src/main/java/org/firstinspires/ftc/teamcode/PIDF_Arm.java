...
public class PIDF_Arm extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            //get velocity
//            double left_slidePosition = left_slide.getCurrentPosition();
            double right_slidePosition = right_slide.getCurrentPosition();

            // Use our controller to calculate p, i, and d
            controller.setPID(p, i, d);
            double pidLeft = controller.calculate(right_slidePosition, target);

            if (right_slidePosition > -1000 && right_slidePosition < 5) {
                left_slide.setPower(pidLeft);
                right_slide.setPower(pidLeft);
            }
            telemetry.addData("target", target);
            telemetry.addData("left arm power", left_slide.getPower());
            telemetry.addData("right arm power", right_slide.getPower());
            telemetry.addData("left arm pos", left_slide.getCurrentPosition());
            telemetry.addData("right arm pos", right_slide.getCurrentPosition());
            telemetry.update();
            //        }
        }
    }
